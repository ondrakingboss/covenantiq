...
export const metadata: Metadata = {
  ...
  description: "Public-beta private-credit analysis with traceable debt, covenant, sensitivity, and recommendation calculations.",
};
  ...
  return <html lang="en"><body className="font-sans antialiased">
    <header className="no-print sticky top-0 z-50 border-b border-ink/20 bg-paper">
      <nav className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-5 lg:px-8">
        ...
        <div className="flex items-center gap-3 text-sm sm:gap-5"><span className="border border-accent/40 bg-accent/5 px-2 py-1 font-mono text-[9px] font-semibold uppercase tracking-wider text-accent">Public beta</span><Link href="/demos" className="hover:text-accent">Workflows</Link><Link href="/borrowers" className="hover:text-accent">Borrowers</Link><Link href="/analyses" className="hidden hover:text-accent sm:inline">Saved analyses</Link><span className="hidden font-mono text-[10px] text-ink/55 lg:inline">CREDIT RISK WORKBENCH</span></div>
      </nav>
    </header>{children}
  </body></html>;
}
